...
from mf_logging import *
from mf_errors import *


class Query:
    def __init__(self, title):
        self.movie = Movie(title)
        self.seance_date = datetime.today()
        self.seance_time = None
...
class MovieFinder:
    ...
    def find_movie_id(self, query):
        ...
        query_date = '-'.join([self.query.seance_date.strftime('%d'), self.afisha_date_mapper[query.seance_date.strftime('%m')]])
    ...
    def get_movie_schedule(self, query):
        url = self.movie_schedule_url.format(query.movie.id, query.seance_date.strftime('%d-%m-%Y'))
        try:
            page = requests.get(url)
        except FailedRequestError as e:
            self.logger.error(e)
            ...
            for theatre_schedule in search_results:
                ...
                for seance in timetable.findall('li'):
                    ...
                    if len(movie_format) > 0:
                        for x in movie_format[0].text.split(' '):
                            if 'D' in x:
                                ms.format = x
...
def get_seance_list(movie_name):
